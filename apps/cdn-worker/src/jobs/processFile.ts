...
export const processFile = async (job: ProcessFileJob) => {
  const { fileId, storageKey, requestedSizes } = job;

  const result = measureJobDuration("processFile", "success", async () => {
    try {
      // 1. Get file metadata
      ...
      // 2. Download raw file
      const raw = await storageService.downloadFile(
        config.S3_BUCKET_RAW,
        storageKey
      );

      logger.info(
        `Worker: downloaded raw file ${storageKey}, size: ${raw.length} bytes`
      );
      if (raw.length > 0) {
        logger.info(
          `Worker: first 20 bytes: ${raw.subarray(0, 20).toString("hex")}`
        );
      } else {
